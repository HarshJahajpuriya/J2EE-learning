...
function fetchEmployees() {
  employees = []
  sendXHR("/stylethree/getEmployees", 'GET')
    .then(response => {
      let employeeStrs = response.split(':');
...
function showEmployees() {
  ...
  for (let employee of employees) {
    let htmlStr = `<tr style='cursor:pointer' onclick="selectEmployee(this, '${employee.employeeId}')">
                    ...
                    <td style='text-align:center'><a href="/stylethree/editEmployee?employeeId=${employee.employeeId}">Edit</a></td>
                    <td style='text-align:center'><a href="/stylethree/confirmDeleteEmployee?employeeId=${employee.employeeId}">Delete</a></td>
                  </tr>`
    index++;
