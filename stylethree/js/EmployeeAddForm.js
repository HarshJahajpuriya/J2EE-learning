...
.then(response => {
  ...
  sendXHR("/stylethree/getEmployee", 'POST', "employeeId="+encodeURI(employeeId))
  .then(response => {
    console.log(form);
...
function addEmployee() {
  let dataToSend = "name="+encodeURI(form.name.value);
  ...
  sendXHR("/stylethree/addEmployee", 'POST', dataToSend)
  .then(response => {
    showNotification(
      'Add more?',
      'Ok',
      '/stylethree/EmployeeAddForm.html',
      'No',
    ...
    console.log(err);
    redirect('/stylethree/Employees.html');
  })
}